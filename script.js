...
function renderizarPresentes() {
    ...
    presentesOrdenados.forEach(item => {
        ...
        card.innerHTML = `
            ...
            <div class="card-info">
                <h3>${item.nome}</h3>
                <button type="button" class="btn-presentear" data-nome="${item.nome.replace(/"/g, '&quot;')}">Presentear</button>
            </div>
        `;
...
// Delegação de clique: abre o modal de Pix ao clicar em qualquer botão "Presentear"
if (container) {
    container.addEventListener('click', (event) => {
        const botao = event.target.closest('.btn-presentear');
        if (botao) {
            abrirModalPresente(botao.dataset.nome);
        }
    });
}
...
/* ===== Modal "Presentear" (Pix com valor escolhido) ===== */
let presenteSelecionado = '';

function abrirModalPresente(nome) {
    presenteSelecionado = nome;

    const modal = document.getElementById('modal-presente');
    const nomeEl = document.getElementById('pix-modal-nome-presente');

    if (!modal || !nomeEl) return;

    nomeEl.textContent = nome;
    atualizarLinksWhatsPresente();
...
function atualizarLinksWhatsPresente() {
    const mensagem = `Olá! Acabei de fazer o Pix para presentear vocês com: ${presenteSelecionado}. 💛`;
    const mensagemCodificada = encodeURIComponent(mensagem);

    const linkAna = document.getElementById('link-whats-ana');
    const linkLucas = document.getElementById('link-whats-lucas');
    if (linkAna) linkAna.href = `https://wa.me/5532998208850?text=${mensagemCodificada}`;
    if (linkLucas) linkLucas.href = `https://wa.me/553298308169?text=${mensagemCodificada}`;
}

function copiarPixModal() {
    const chave = document.getElementById('pix-chave-modal').innerText;
    navigator.clipboard.writeText(chave).then(() => {
        const msg = document.getElementById('msg-copiado-modal');
        msg.style.display = 'block';
        setTimeout(() => { msg.style.display = 'none'; }, 2000);
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    const modalPresente = document.getElementById('modal-presente');

    if (modalPresente) {
        modalPresente.addEventListener('click', (event) => {
